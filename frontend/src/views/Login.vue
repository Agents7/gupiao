    ...
    <div class="flex items-center justify-center p-8 bg-background">
      <div class="w-full max-w-[420px]">
        ...
        <div class="text-center mb-10">
          ...
          <p class="text-muted-foreground text-sm">请输入您的账户信息</p>
        </div>
        ...
        <form @submit.prevent="handleSubmit" class="space-y-5">
          ...
          <div class="flex items-center justify-between">
            <div class="flex items-center space-x-2">
              <AppCheckbox id="remember" v-model="form.remember" />
        ...
        <div class="text-center text-sm text-muted-foreground mt-8">
          还没有账号？
          <router-link to="/signup" class="text-foreground font-medium hover:underline">立即注册</router-link>
        </div>
      </div>
    </div>
  </div>
...
async function handleSubmit() {
  ...
  try {
    ...
    if (err.response) {
      ...
      error.value = detail || "用户名或密码错误，请重试。";
    } else {
